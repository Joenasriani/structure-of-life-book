...
def metadata_paths(m):
 defaults={'manifest':'/publication.json','release':'/release.json','sitemap':'/sitemap.xml','llms':'/llms.txt'}
 return {**defaults,**m.get('public_metadata',{})}
def release_relative_path():
 return metadata_paths(read('publication.json'))['release'].lstrip('/') if (ROOT/'publication.json').exists() else 'release.json'
def release_file():return ROOT/'dist'/release_relative_path()
def verification_files(release,base):
 files=release['files']+[{'path':release_relative_path(),'sha256':digest(release_file().read_bytes())}]
 if (ROOT/'publication.json').exists():
  m=read('publication.json')
  if m.get('routing',{}).get('mode')=='store_reverse_proxy' and base.rstrip('/')==origin(m):
   # The store owns its origin-wide robots policy; verify the publication's robots file on its dedicated project.
   files=[f for f in files if f['path']!='robots.txt']
 return files
def write_public(d,path,text):
 target=d/path.lstrip('/');target.parent.mkdir(parents=True,exist_ok=True);target.write_text(text)
def paypal_target(m):
 c=m['checkout'];hosted=c.get('hosted_url')
 if not hosted:return None
 u=urlsplit(hosted);q=parse_qs(u.query)
 require(u.scheme=='https' and u.netloc=='www.paypal.com' and not u.fragment,'Invalid hosted PayPal origin')
 hosted_button=u.path=='/cgi-bin/webscr' and set(q)=={'cmd','hosted_button_id'} and q['cmd']==['_s-xclick'] and len(q['hosted_button_id'])==1 and re.fullmatch('[A-Z0-9]{8,32}',q['hosted_button_id'][0])
 payment_link=re.fullmatch('/ncp/payment/[A-Z0-9]{8,32}',u.path) and not u.query
 require(hosted_button or payment_link,'Use an official product-specific PayPal button or payment link')
 return hosted
...
def validate_manifest(m):
 ...
 c=m['checkout'];paypal_target(m);require(re.fullmatch(r'\d+\.\d{2}',c['amount']) is not None,'Invalid amount')
 ...
 for path in metadata_paths(m).values():require(path.startswith('/') and not path.startswith('//') and '..' not in path and '?' not in path and '#' not in path,'Invalid public metadata path')
...
def build_book(m,d):
 ...
 for name in ['robots.txt','llms.txt']:
  if (ROOT/name).exists():write_public(d,metadata_paths(m)['llms'] if name=='llms.txt' else '/robots.txt',(ROOT/name).read_text())
 sitemap='<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n'+''.join('<url><loc>'+html.escape(origin(m)+r['path'])+'</loc></url>\n' for r in m['routes'] if r['indexable'])+'</urlset>\n'
 write_public(d,metadata_paths(m)['sitemap'],sitemap);write_public(d,metadata_paths(m)['manifest'],(ROOT/'publication.json').read_text())
 return [m]
def load_catalogue():
...
def build():
 ...
 code=[{'path':p.relative_to(ROOT).as_posix(),'sha256':digest(p.read_bytes())} for p in sorted([*(ROOT/'api').rglob('*.js'),*(ROOT/'lib').rglob('*.js')])] if (ROOT/'api').exists() else []
 save(d/release_relative_path(),{'schema_version':'1.0.0','status':'built_not_deployed','publication_ids':[m['id'] for m in ms],'manifests':{m['id']:digest((ROOT/'publication.json').read_bytes()) if (ROOT/'publication.json').exists() else next(e['sha256'] for e,x in load_catalogue() if x['id']==m['id']) for m in ms},'files':files,'serverless_sources':code,'verification':{'payment':'not_verified','buyer_receipt':'not_verified'}})
...
def local_validate():
 ...
 require(release_file().exists(),'Build before validation')
 for f in json.loads(release_file().read_text())['files']:
...
def verify(base,report,source_repository,source_commit):
 ...
 release=json.loads(release_file().read_text());checks=[]
 expected_files=verification_files(release,base)
 def check_file(f):
 ...
 if (ROOT/'publication.json').exists():
  ...
  try:
   ...
   hosted=paypal_target(m);location=headers.get('Location',headers.get('location',''))
   matched=location==hosted if hosted else target.scheme=='https' and target.netloc=='www.paypal.com' and all(q.get(k)==[v] for k,v in expected.items())
   checks.append({'path':c['endpoint'],'status':status,'expected_location':hosted,'pass':status==302 and matched})
  except Exception as e:checks.append({'path':c['endpoint'],'pass':False,'error':str(e)})
 data={'verified_at':datetime.datetime.now(datetime.timezone.utc).isoformat(),'source_repository':source_repository,'source_commit':source_commit,'publication_manifest_sha256':digest((ROOT/'publication.json').read_bytes()) if (ROOT/'publication.json').exists() else None,'deployment_base':base,'expected_release_sha256':digest(release_file().read_bytes()),'pass':all(x['pass'] for x in checks),'checks':checks,'payment_acceptance':'not_tested','buyer_receipt':'not_tested'}
...
def gate():
 ...
 require(evidence.get('expected_release_sha256')==digest(release_file().read_bytes()),'Preview evidence is stale')
 ...
 expected=verification_files(json.loads(release_file().read_text()),evidence['deployment_base'])
 for f in expected:
  path='/'+f['path'];path='/' if path=='/index.html' else path[:-5] if path.endswith('.html') and not path.startswith('/google') and path!='/404.html' else path
